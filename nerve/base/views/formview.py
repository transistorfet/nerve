...
class FormView (nerve.View):
    ...
    def __init__(self, config_info, data, target=None, submit='Save', submitback=False):
        super().__init__()
        self._data = data
    ...
    def build_item(self, basename, name, propername, datatype, data, parenttype=None):
        ...
        if datatype.is_type('scalar'):
            ...
            self.write_text('<label class="nerve-treeview{0}" for="{1}"><span>{2}</span></label>\n'.format(alt_label, pathjoin(basename, name), propername))
    ...
    def build_complex_item(self, basename, name, propername, datatype, data):
        ...
        if datatype.is_type('container'):
            ...
            self.build_item(pathjoin(basename, name), '__new__', "(new)", datatype.itemtype, None, datatype)
            self.write_text('</div>\n', indent=-1)
